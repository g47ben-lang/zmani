...
| `--min-duration N` | דלג על תוצאות קצרות מ-`N` שניות והורד את הראשונה שארוכה מספיק. |
...
**מוריד קליפ קצר במקום הגרסה המלאה/הארוכה?**
הוסף `--min-duration` עם מינימום אורך **בשניות**, והסקריפט ידלג על התוצאות
הקצרות ויוריד את הראשונה שארוכה מספיק. למשל לפחות 10 דקות:

```bash
python3 download_songs.py --list songs.txt --link --min-duration 600
```

`600` = 10 דקות, `1800` = חצי שעה, `3600` = שעה. אפשר גם דרך משתנה סביבה:
`MIN_DURATION=600`. שים לב: זה רק *בוחר* מבין התוצאות הקיימות — אם לפריט
מסוים אין בכלל גרסה ארוכה, הוא ייכשל עליו במקום להוריד קליפ קצר.
